...
function EtablissementsContent() {
  ...
  const hasActiveFilters = search || secteur || communeId || annexeId || noteMin > 0;
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            >
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200/60 p-5 sticky top-24">
                ...
                {/* Annexe Filter (Conditional) */}
                {communeId && annexes.length > 0 && (
                <div className="mb-6">
                  <label className={`text-xs font-semibold text-gray-500 mb-2 block ${locale === 'ar' ? '' : 'uppercase tracking-wider'}`}>{t('filters.annexe') || 'Annexe'}</label>
                  <select
                    value={annexeId}
