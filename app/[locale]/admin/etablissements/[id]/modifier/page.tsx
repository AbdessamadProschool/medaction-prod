...
export default function ModifierEtablissementPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (communesRes.ok) {
          const json = await communesRes.json();
          setCommunes(json.communes || []);
        }

        // Load etablissement
        const res = await fetch(`/api/etablissements/${id}`);
        if (!res.ok) throw new Error("Erreur lors du chargement de l'établissement");
        ...
        setFormData({
          ...
          code: data.code || '',
          secteur: data.secteur || 'EDUCATION',
          typeEtablissement: data.typeEtablissement || '',
          nature: data.nature || 'PUBLIC',
          tutelle: data.tutelle || '',
          statutJuridique: data.statutJuridique || '',
          gestionnaire: data.gestionnaire || '',
          responsableNom: data.responsableNom || '',
          anneeCreation: data.anneeCreation ? String(data.anneeCreation) : '',
          anneeOuverture: data.anneeOuverture ? String(data.anneeOuverture) : '',
          
          communeId: data.communeId ? String(data.communeId) : '',
          annexeId: data.annexeId ? String(data.annexeId) : '',
          quartierDouar: data.quartierDouar || '',
          adresseComplete: data.adresseComplete || '',
          latitude: data.latitude ? String(data.latitude) : '',
          longitude: data.longitude ? String(data.longitude) : '',
          altitude: data.altitude ? String(data.altitude) : '',
          
          telephone: data.telephone || '',
          email: data.email || '',
          siteWeb: data.siteWeb || '',
          
          etatInfrastructure: data.etatInfrastructure || 'BON',
          statutFonctionnel: data.statutFonctionnel || 'FONCTIONNEL',
          surfaceTotale: data.surfaceTotale ? String(data.surfaceTotale) : '',
          disponibiliteEau: data.disponibiliteEau ?? true,
          disponibiliteElectricite: data.disponibiliteElectricite ?? true,
          connexionInternet: data.connexionInternet ?? true,
          nombreSalles: data.nombreSalles ? String(data.nombreSalles) : '',
          
          effectifTotal: data.effectifTotal ? String(data.effectifTotal) : '',
          nombrePersonnel: data.nombrePersonnel ? String(data.nombrePersonnel) : '',
          cadre: data.cadre || '',
          capaciteAccueil: data.capaciteAccueil ? String(data.capaciteAccueil) : '',
          
          cycle: data.cycle || '',
          nbClasses: data.nbClasses ? String(data.nbClasses) : '',
          nbEnseignants: data.nbEnseignants ? String(data.nbEnseignants) : '',
          nbCadres: data.nbCadres ? String(data.nbCadres) : '',
          elevesPrescolaire: data.elevesPrescolaire ? String(data.elevesPrescolaire) : '',
          elevesTotal: data.elevesTotal ? String(data.elevesTotal) : '',
          nouveauxInscrits: data.nouveauxInscrits ? String(data.nouveauxInscrits) : '',
          tauxReussite: data.tauxReussite ? String(data.tauxReussite) : '',
          
          sourcesFinancement: data.sourcesFinancement || '',
          budgetAnnuel: data.budgetAnnuel ? String(data.budgetAnnuel) : '',
          partenaires: data.partenaires || '',
          
          isPublie: data.isPublie || false,
          isValide: data.isValide ?? true
        });
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const submitPromise = new Promise(async (resolve, reject) => {
      try {
        await actionMutation.mutate(`/api/etablissements/${id}`, {
          method: 'PATCH',
          data: {
            ...formData,
  ...
  // Fallback communes list if dynamic fetch is empty
  const communeOptions = communes.length > 0
    ? communes.map(c => ({ label: c.nom, value: String(c.id) }))
    : [
