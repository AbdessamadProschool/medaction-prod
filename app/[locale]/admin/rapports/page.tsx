    ...
    <div className="p-6 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-4 items-center bg-white p-2 rounded-lg border border-gray-200 shadow-sm">
          ...
          />
          <span className="text-gray-400">{t('date_range_to')}</span>
          <input 
            type="date" 
      ...
      <div className="flex gap-4">
        ...
        >
            <FileSpreadsheet className="w-4 h-4" /> {t('export_buttons.excel_reclamations')}
        </button>
        <button 
            onClick={() => handleExport('excel', 'evenements')}
            className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
        >
            <FileSpreadsheet className="w-4 h-4" /> {t('export_buttons.excel_events')}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-gray-500 text-sm font-medium">{t('kpi_cards.total_reclamations')}</h3>
          <p className="text-3xl font-bold text-gray-900 mt-2">{reclamationsData?.total || 0}</p>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-gray-500 text-sm font-medium">{t('kpi_cards.total_events')}</h3>
          <p className="text-3xl font-bold text-gray-900 mt-2">{evenementsData?.total || 0}</p>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-gray-500 text-sm font-medium">{t('kpi_cards.average_rating')}</h3>
          <p className="text-3xl font-bold text-gray-900 mt-2">
            {satisfactionData?.global?.average?.toFixed(1) || 'N/A'} <Star className="w-4 h-4 text-yellow-500 inline" />
          </p>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-gray-500 text-sm font-medium">{t('kpi_cards.event_participation')}</h3>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-900 mb-6">{t('charts.reclamations_by_status')}</h3>
          <div className="h-80">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-900 mb-6">{t('charts.reclamations_evolution')}</h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-900 mb-6">{t('charts.reclamations_by_commune')}</h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-900 mb-6">{t('charts.top_5_establishments')}</h3>
            ...
            {satisfactionData?.topEtablissements?.map((etab: any, i: number) => (
                <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-400">{t('reviews_count', { count: etab.nombreEvaluations })}</span>
                    </div>
                </div>
            ))}
