...
export default function EditActualitePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const fetchActualite = async () => {
       try {
         const res = await fetch(`/api/delegation/actualites/${id}`);
         if (res.ok) {
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-300">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-3">
                <label className="block text-sm font-bold text-gray-800 text-start">
                ...
                <div className="relative group">
                    <input
                      ...
                      className="w-full px-4 py-2.5 pr-10 rtl:pr-10 rtl:pl-4 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 outline-none transition-all text-base font-semibold placeholder:text-gray-300 placeholder:font-normal bg-gray-50/50 focus:bg-white text-sm text-start"
                    />
                     <div className="absolute right-5 rtl:right-5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-500 transition-colors pointer-events-none">
                        <LayoutTemplate size={24} />
                     </div>
                </div>
                {errors.titre && <p className="text-red-500 text-sm font-bold flex items-center gap-2 bg-red-50 p-2.5 rounded-xl max-w-fit px-4 text-start"><X size={16}/> {errors.titre.message}</p>}
              </div>
              ...
              <div className="space-y-3">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 outline-none transition-all placeholder:text-gray-300 bg-gray-50/50 focus:bg-white text-sm font-medium resize-none text-start"
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="relative rounded-2xl border-2 border-gray-100 focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-500/10 transition-all bg-gray-50/50 focus-within:bg-white overflow-hidden">
                    ...
                    className="w-full px-4 py-3 outline-none bg-transparent placeholder:text-gray-300 leading-relaxed text-sm font-medium resize-y text-start"
                    />
                </div>
                {errors.contenu && <p className="text-red-500 text-sm font-bold flex items-center gap-2 bg-red-50 p-2.5 rounded-xl max-w-fit px-4 text-start"><X size={16}/> {errors.contenu.message}</p>}
              </div>
            </div>
          </div>
