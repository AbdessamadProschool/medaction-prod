  ...
  useEffect(() => {
    const fetchEtablissements = async () => {
      try {
        ...
        toast.error('Erreur chargement des établissements');
      }
    };
    ...
    try {
      ...
      if (selectedImage) {
        ...
        if (!uploadRes.ok) {
          ...
          throw new Error(errData.error || t('errors.upload_failed'));
        }
      ...
      const res = await fetch('/api/delegation/actualites', {
        ...
        body: JSON.stringify({
            ...
            isPublie: true // Default to published or handle draft later
        }),
      });
      ...
      console.error(error);
      toast.error('Erreur: ' + (error instanceof Error ? error.message : t('errors.create_failed')));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="mb-5 relative">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-gray-100">
             <div className="space-y-2">
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-orange-50 rounded-full border border-orange-100 animate-fade-in-up">
                    <span className="w-2 h-2 rounded-full bg-orange-500 animate-pulse"></span>
                    <span className="text-xs font-bold text-orange-700 uppercase tracking-wider">{t('subtitle')}</span>
                </div>
                <h1 className="text-2xl font-black text-gray-900 tracking-tight leading-tight">
                    {t('title')}
                </h1>
             </div>
             <div className="w-10 h-10 bg-gradient-to-br from-orange-500 to-red-500 rounded-xl flex items-center justify-center shadow-lg shadow-orange-500/30 transform rotate-3">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 animate-fade-in-up animation-delay-100">
                ...
                <div className="relative w-full h-48 bg-gray-100 rounded-xl overflow-hidden group shadow-inner border border-gray-100">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                      <button
                        ...
                        className="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors text-sm font-bold shadow-lg transform hover:scale-105 flex items-center gap-2"
                      >
                        <X size={20} />
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-2">
                <label className="block text-sm font-bold text-gray-700 text-start">
                  {t('sections.content.titre_label')} <span className="text-red-500">*</span>
                </label>
                <div className="relative group">
                    <input
                      ...
                      className="w-full px-4 py-3 pr-12 rtl:pr-12 rtl:pl-4 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 outline-none transition-all text-sm font-bold placeholder:text-gray-300 placeholder:font-normal bg-gray-50/50 focus:bg-white text-start"
                    />
                     <div className="absolute right-6 rtl:right-6 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-500 transition-colors pointer-events-none">
                        <LayoutTemplate size={20} />
                     </div>
                </div>
                {errors.titre && <p className="text-red-500 text-xs font-bold flex items-center gap-2 bg-red-50 p-2 rounded-xl max-w-fit px-4 text-start"><X size={14}/> {errors.titre.message}</p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 outline-none transition-all text-sm font-bold placeholder:text-gray-300 bg-gray-50/50 focus:bg-white text-start resize-none"
                />
              </div>
              ...
              <div className="space-y-2">
                <label className="block text-sm font-bold text-gray-700 text-start">
                   {t('sections.content.body_label')} <span className="text-red-500">*</span>
                </label>
                <div className="relative rounded-xl border border-gray-200 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/10 transition-all bg-gray-50/50 focus-within:bg-white overflow-hidden">
                    <textarea
                    {...register('contenu')}
                    rows={6}
                ...
                {errors.contenu && <p className="text-red-500 text-xs font-bold flex items-center gap-2 bg-red-50 p-2 rounded-xl max-w-fit px-4 text-start"><X size={14}/> {errors.contenu.message}</p>}
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all duration-300 relative overflow-hidden group">
             <div className="absolute top-0 right-0 rtl:right-auto rtl:left-0 w-32 h-32 bg-purple-50 rounded-bl-[4rem] rtl:rounded-bl-none rtl:rounded-br-[4rem] -mr-8 -mt-8 rtl:-mr-0 rtl:-ml-8 opacity-50 pointer-events-none group-hover:scale-110 transition-transform"></div>
              <h3 className="text-sm font-bold text-gray-700 mb-4 flex items-center gap-3">
              ...
              <div className="relative group">
                 <select
                    {...register('categorie')}
                     className="w-full px-4 py-3 pr-10 pl-6 rounded-xl border border-gray-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/10 outline-none bg-white font-bold text-gray-700 appearance-none cursor-pointer transition-all hover:bg-gray-50 text-sm text-start"
                 >
                 ...
                 <div className="absolute right-6 rtl:right-auto rtl:left-6 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none group-hover:text-purple-600 transition-colors">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all duration-300 relative overflow-hidden group">
               <div className="absolute top-0 right-0 rtl:right-auto rtl:left-0 w-32 h-32 bg-indigo-50 rounded-bl-[4rem] rtl:rounded-bl-none rtl:rounded-br-[4rem] -mr-8 -mt-8 rtl:-mr-0 rtl:-ml-8 opacity-50 pointer-events-none group-hover:scale-110 transition-transform"></div>
              <h3 className="text-sm font-bold text-gray-700 mb-4 flex items-center gap-3">
              ...
              <div className="relative group">
                 <select
                    {...register('etablissementId')}
                     className="w-full px-4 py-3 pr-10 pl-6 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 outline-none bg-white font-bold text-gray-700 appearance-none cursor-pointer transition-all hover:bg-gray-50 text-sm text-start"
                 >
                 ...
                 <div className="absolute right-6 rtl:right-auto rtl:left-6 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none group-hover:text-indigo-600 transition-colors">
                    <ArrowRight className="rotate-90" size={20} />
                 </div>
              </div>

              {errors.etablissementId && <p className="text-red-500 text-sm font-bold mt-3 flex items-center gap-2 text-start"><X size={16}/> {errors.etablissementId.message}</p>}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-10 border-t border-gray-200">
            <Link
              href="/delegation/actualites"
              className="px-6 py-3 text-gray-500 hover:text-gray-900 font-bold transition-all hover:bg-gray-100 rounded-xl text-sm"
            >
              {t('actions.cancel')}
            </Link>
