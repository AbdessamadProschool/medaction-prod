            ...
            <div className="p-8">
              {previewUrl ? (
                <div className="relative w-full h-72 bg-gray-100 rounded-[1.5rem] overflow-hidden group shadow-inner border border-gray-100">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                      <span className="text-white font-bold text-lg">{t('sections.cover.selected')}</span>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="px-6 py-3 bg-red-500 text-white rounded-xl hover:bg-red-600 transition-colors font-bold shadow-lg"
                      >
                        {t('sections.cover.remove')}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <label className="relative flex flex-col items-center justify-center h-72 border-3 border-dashed border-gray-200 rounded-[1.5rem] bg-gray-50/50 hover:bg-blue-50/50 hover:border-blue-300 transition-all cursor-pointer group overflow-hidden">
                  ...
                  <p className="text-xl font-bold text-gray-700 mb-2 group-hover:text-blue-700 transition-colors">{t('sections.cover.click_to_add')}</p>
                  <p className="text-gray-400 font-medium">{t('sections.cover.formats')}</p>
                  <input 
                    type="file" 
                    className="hidden"
                    accept="image/*"
            ...
            <div className="p-8 space-y-8">
              ...
              <div className="space-y-4">
                <label className="block text-lg font-bold text-gray-900 text-right" style={{ textAlign: 'right' }}>
                  {t('sections.content.article_title')} <span className="text-red-500 text-lg">*</span>
                </label>
              ...
              <div className="space-y-4">
                <label className="block text-lg font-bold text-gray-900 text-right" style={{ textAlign: 'right' }}>
                  {t('sections.content.full_content')} <span className="text-red-500 text-lg">*</span>
                </label>
                <textarea
                  {...register('contenu')}
                  rows={8}
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8 hover:shadow-lg transition-all duration-300">
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  <label className="block text-base font-bold text-gray-700 text-right" style={{ textAlign: 'right' }}>{t('sections.metadata.category')}</label>
                  <select
                    {...register('categorie')}
                ...
                <div className="space-y-3">
                  <label className="block text-base font-bold text-gray-700 text-right" style={{ textAlign: 'right' }}>
                    {t('sections.metadata.tags')} <span className="text-gray-400 font-normal text-sm">{t('sections.metadata.tags_hint')}</span>
                  </label>
                  <input
                    {...register('tags')}
                    type="text"
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8 hover:shadow-lg transition-all duration-300">
              ...
              <div className="space-y-4">
                <label className="flex items-start gap-4 p-5 rounded-2xl border-2 border-gray-100 hover:border-purple-400 hover:bg-purple-50/30 cursor-pointer transition-all shadow-sm hover:shadow-md">
                  ...
                  />
                  <div>
                    <span className="font-bold text-gray-800 block text-lg">{t('sections.publication.publish_now')}</span>
                    <span className="text-gray-500 text-sm font-medium">{t('sections.publication.publish_now_desc')}</span>
                  </div>
                </label>
              </div>
            </div>
