...
export default function ClotureCampagnePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Form State
  const [nbParticipations, setNbParticipations] = useState('');
  ...
  useEffect(() => {
    // Charger infos campagne
    fetch(`/api/delegation/campagnes/${campagneId}`)
      .then(res => res.json())
      .then(data => {
        if (data.data) {
            setCampagne(data.data);
            if (data.data.nombreParticipations) setNbParticipations(data.data.nombreParticipations.toString());
            if (data.data.bilanDescription) setBilanDescription(data.data.bilanDescription);
            if (data.data.rapportClotureUrl) setCompteRenduUrl(data.data.rapportClotureUrl);
        }
      })
      .catch(console.error);
  }, [campagneId]);
  ...
  const handleCompteRenduUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append('type', 'CAMPAIGN_REPORT');
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const formData = new FormData();
    formData.append('file', e.target.files[0]);
    formData.append('type', 'CAMPAIGN_BILAN'); 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!nbParticipations || !bilanDescription) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/delegation/campagnes/${campagneId}`, {
        method: 'PUT',
