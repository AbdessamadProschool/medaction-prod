...
export default function ModifierEvenementPage() {
  ...
  const { register, handleSubmit, formState: { errors }, reset, watch, control } = useForm<EventForm>({
    ...
    fetch(`/api/delegation/evenements/${id}`)
      ...
      .then(data => {
        if (data.data) {
          const evt = data.data;
          setEvenement(evt);
          if (evt.lieuEtablissementId) {
            setLocationMode('etablissement');
            if (evt.lieuEtablissement?.secteur) {
               setLieuSecteur(evt.lieuEtablissement.secteur);
            }
          }
          ...
          reset({
            ...
            lieuEtablissementId: evt.lieuEtablissementId?.toString() || '',
          });
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Controller
                    ...
                    render={({ field }) => (
                      <GovDatePicker
                        label={t('sections.datetime.start_date')}
                        value={field.value}
                ...
                <div className="space-y-1.5">
                  <Controller
                    ...
                    render={({ field }) => (
                      <GovDatePicker
                        label={t('sections.datetime.end_date')}
                        value={field.value}
                        onChange={field.onChange}
                ...
                {locationMode === 'etablissement' ? (
                  <div className="grid md:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
                    <div className="space-y-3">
                      <label className="block text-xs font-bold text-gray-600 text-start">Secteur du lieu (optionnel)</label>
                      <select
                        value={lieuSecteur}
