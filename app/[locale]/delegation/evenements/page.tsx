    ...
    <div className="space-y-10">
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-600 text-white shadow-lg shadow-blue-500/10 px-6 py-6 md:px-8 md:py-10">
        ...
        <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
            <Calendar className="w-48 h-48 transform -rotate-12" />
        </div>
        ...
        <div className={`relative z-10 flex flex-col md:flex-row ${direction === 'rtl' ? 'md:items-start text-right' : 'md:items-end text-left'} justify-between gap-6`}>
            <div className={`space-y-3 flex flex-col ${direction === 'rtl' ? 'items-end' : 'items-start'}`}>
                <div className={`flex items-center gap-2.5 text-blue-100 font-bold bg-white/10 w-fit px-3 py-1 rounded-xl backdrop-blur-md border border-white/10 text-xs uppercase tracking-wider ${direction === 'rtl' ? 'flex-row-reverse' : ''}`}>
            ...
            <Link
                ...
                className="group relative px-6 py-3 bg-white text-indigo-700 rounded-xl font-black text-sm shadow-xl hover:shadow-indigo-500/20 hover:-translate-y-1 active:scale-95 transition-all duration-300 flex items-center gap-2 overflow-hidden no-underline"
            >
