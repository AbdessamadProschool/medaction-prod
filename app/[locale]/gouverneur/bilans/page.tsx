...
interface BilanEvenement {
  ...
  etablissement?: { id: number; nom: string; nomArabe?: string };
...
interface BilanActivite {
  ...
  dateRapport?: string;
  etablissement: { id: number; nom: string; nomArabe?: string; secteur: string; commune?: { nom: string; nomArabe?: string } };
  createdByUser?: { nom: string; prenom: string };
}
...
export default function GouverneurBilansPage() {
  ...
  const [activeTab, setActiveTab] = useState<'reports' | 'evenements' | 'activites' | 'campagnes' | 'actualites'>('reports');
        ...
        if (reportWindow) {
          ...
          const htmlContent = `
            ...
            <body>
              ...
              <div class="section">
                 <h2>3. ${isRTL ? 'المشاريع والتواصل' : 'Communication & Projets'}</h2>
                 <div class="grid">
                    <div class="card">
              ...
              <h2>4. ${isRTL ? 'التوزيع حسب الجماعة' : 'Détail par Commune'}</h2>
                 <table class="table">
                    <thead><tr><th>${isRTL ? 'الجماعة' : 'Commune'}</th><th>${isRTL ? 'عدد الملفات' : 'Dossiers'}</th></tr></thead>
                    <tbody>
                       ${d.communes.map((c: any) => `<tr><td>${isRTL ? (c.nomArabe || c.nom) : c.nom}</td><td><strong>${c.count}</strong></td></tr>`).join('')}
                    </tbody>
                 </table>
              </div>

               <div class="section">
                 <h2>5. ${isRTL ? 'التوزيع القطاعي' : 'Répartition Sectorielle'}</h2>
                 <table class="table">
    ...
    return (
      <div className="mt-4">
        <h5 className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
          ...
          {t('reports.bilans_page.labels.photos')} ({photos.length})
        </h5>
  ...
  const MediaGallery = ({ medias }: { medias?: MediaItem[] }) => {
    ...
    return (
      <div className="mt-4">
        <h5 className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
          <ImageIcon className="w-4 h-4" />
          {t('reports.bilans_page.labels.photos')} ({images.length})
        </h5>
        <div className="flex flex-wrap gap-2">
          {images.map((media) => (
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-4">{t('reports.bilans_page.loading')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            {t('reports.bilans_page.subtitle')}
          </p>
        </div>
      </div>
     ...
     {/* Stats Overview */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-5 text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-blue-100">{t('reports.bilans_page.stats.events')}</p>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-5 text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-purple-100">{t('reports.bilans_page.stats.activities')}</p>
        ...
        <div className="bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl p-5 text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-amber-100">{t('reports.bilans_page.stats.campaigns')}</p>
        ...
        <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-xl p-5 text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-emerald-100">{t('reports.bilans_page.stats.participants')}</p>
      ...
      <div className="flex border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
        ...
        >
          ...
          {t('reports.bilans_page.tabs.events')} ({evenements.length})
        </button>
        <button
        ...
        >
          ...
          {t('reports.bilans_page.tabs.activities')} ({activites.length})
        </button>
        <button
        ...
        >
          ...
          {t('reports.bilans_page.tabs.campaigns')} ({campagnes.length})
        </button>
      </div>
      ...
      {activeTab !== 'reports' && (
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder={t('reports.bilans_page.search')}
              value={searchQuery}
          ...
          {(activeTab === 'evenements' || activeTab === 'activites') && (
            ...
            >
              <option value="">{t('reports.bilans_page.sectors_all')}</option>
        ...
        <div className="space-y-4">
          {filteredEvenements.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
              ...
              <h3 className="text-lg font-medium text-gray-700">{t('reports.bilans_page.empty.events')}</h3>
              <p className="text-gray-500">{t('reports.bilans_page.empty.desc')}</p>
            </div>
          ) : (
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      {evt.medias && evt.medias.length > 0 && (
                        <span className="text-xs text-emerald-600 flex items-center gap-1">
                          ...
                          {evt.medias.filter(m => m.type === 'IMAGE').length} {t('reports.bilans_page.labels.photos')}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-4">
                      {evt.etablissement && (
                        <span className="flex items-center gap-1">
                          ...
                          {isRTL ? (evt.etablissement.nomArabe || evt.etablissement.nom) : evt.etablissement.nom}
                        </span>
                      )}
                      {evt.commune && (
                        <span className="flex items-center gap-1">
                          <MapPin className="w-4 h-4" />
                          {isRTL ? (evt.commune.nomArabe || evt.commune.nom) : evt.commune.nom}
                        </span>
                      )}
                      <span className="flex items-center gap-1 font-medium text-emerald-600">
                        <Users className="w-4 h-4" />
                        {evt.bilanNbParticipants || evt.nombreInscrits} {t('reports.bilans_page.labels.participations')}
                      </span>
                    </div>
                      ...
                      <div className="bg-gradient-to-r from-gray-50 to-emerald-50 dark:bg-gray-700 rounded-lg p-4 border-l-4 border-emerald-500">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                            ...
                            {t('reports.bilans_page.labels.closing_report')}
                          </h4>
                          <button 
                          ...
                          >
                            ...
                            {isExpanded ? t('reports.bilans_page.labels.reduce') : t('reports.bilans_page.labels.view_details')}
                          </button>
                        </div>
                        ...
                        {evt.bilanDatePublication && (
                          <p className="text-xs text-gray-400 mt-3">
                            📅 {t('reports.bilans_page.labels.published_on')} {new Date(evt.bilanDatePublication).toLocaleDateString(locale)}
                          </p>
                        )}
        ...
        <div className="space-y-4">
          {filteredActivites.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
              ...
              <h3 className="text-lg font-medium text-gray-700">{t('reports.bilans_page.empty.activities')}</h3>
              <p className="text-gray-500">{t('reports.bilans_page.empty.desc')}</p>
            </div>
          ) : (
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      {act.photosRapport && act.photosRapport.length > 0 && (
                        <span className="text-xs text-emerald-600 flex items-center gap-1">
                          <ImageIcon className="w-3 h-3" />
                          {act.photosRapport.length} {t('reports.bilans_page.labels.photos')}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-4">
                      <span className="flex items-center gap-1">
                        <Building2 className="w-4 h-4" />
                        {isRTL ? (act.etablissement.nomArabe || act.etablissement.nom) : act.etablissement.nom}
                      </span>
                      {act.etablissement.commune && (
                        <span className="flex items-center gap-1">
                          <MapPin className="w-4 h-4" />
                          {isRTL ? (act.etablissement.commune.nomArabe || act.etablissement.commune.nom) : act.etablissement.commune.nom}
                        </span>
                      )}
                      <span className="flex items-center gap-1 font-medium text-emerald-600">
                        <Users className="w-4 h-4" />
                        {act.presenceEffective || 0} / {act.participantsAttendus || '?'} {t('reports.bilans_page.labels.present')}
                        {act.tauxPresence && (
                    ...
                    <div className="bg-gradient-to-r from-purple-50 to-blue-50 dark:bg-gray-700 rounded-lg p-4 border-l-4 border-purple-500">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                          ...
                          {t('reports.bilans_page.labels.activity_report')}
                        </h4>
                        <button 
                      ...
                      {act.commentaireDeroulement && (
                        <p className="text-gray-700 dark:text-gray-300 text-sm mb-3">
                          <strong>{t('reports.bilans_page.labels.progress')} :</strong> {isExpanded ? act.commentaireDeroulement : (act.commentaireDeroulement.length > 200 ? act.commentaireDeroulement.substring(0, 200) + '...' : act.commentaireDeroulement)}
                        </p>
                      )}
                        ...
                        <>
                          {act.pointsPositifs && (
                            <p className="text-emerald-700 dark:text-emerald-400 text-sm mb-2 flex items-start gap-2">
                              <strong className="flex items-center gap-1"><CheckCircle2 className="w-4 h-4" /> {t('reports.bilans_page.labels.positives')} :</strong> {act.pointsPositifs}
                            </p>
                          )}
                          {act.difficultes && (
                            <p className="text-amber-700 dark:text-amber-400 text-sm mb-2 flex items-start gap-2">
                              <strong className="flex items-center gap-1"><AlertCircle className="w-4 h-4" /> {t('reports.bilans_page.labels.difficulties')} :</strong> {act.difficultes}
                            </p>
                          )}
                          {act.recommandations && (
                            <p className="text-blue-700 dark:text-blue-400 text-sm mb-2 flex items-start gap-2">
                              <strong className="flex items-center gap-1"><Lightbulb className="w-4 h-4" /> {t('reports.bilans_page.labels.recommendations')} :</strong> {act.recommandations}
                            </p>
                          )}
                      ...
                      {act.dateRapport && (
                        <p className="text-xs text-gray-400 mt-2">
                          {t('reports.bilans_page.labels.report_completed_on')} {new Date(act.dateRapport).toLocaleDateString(locale)}
                        </p>
                      )}
                    </div>
                  </div>
      ...
      {activeTab === 'campagnes' && (
        <div className="space-y-4">
          {filteredCampagnes.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
              <Megaphone className="w-16 h-16 text-gray-300 mx-auto mb-4" />
              <h3 className="text-lg font-medium text-gray-700">{t('reports.bilans_page.empty.campaigns')}</h3>
              <p className="text-gray-500">{t('reports.bilans_page.empty.desc')}</p>
            </div>
          ) : (
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <span className="px-2 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-700">
                        {t('reports.bilans_page.labels.citizen_campaign')}
                      </span>
                      {camp.dateDebut && (
                        <span className="text-xs text-gray-500 flex items-center gap-1">
                          <Clock className="w-3 h-3" />
                      ...
                      {camp.medias && camp.medias.length > 0 && (
                        <span className="text-xs text-emerald-600 flex items-center gap-1">
                          <ImageIcon className="w-3 h-3" />
                          {camp.medias.filter(m => m.type === 'IMAGE').length} {t('reports.bilans_page.labels.photos')}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 text-sm mb-4">
                      <span className="flex items-center gap-1 font-medium text-emerald-600">
                        <Users className="w-4 h-4" />
                        {camp.nombreParticipations} {t('reports.bilans_page.labels.participations')}
                      </span>
                      <span className="flex items-center gap-1 text-gray-500">
                        <Eye className="w-4 h-4" />
                        {camp.nombreVues} {t('reports.bilans_page.labels.views')}
                      </span>
                      {camp.objectifParticipations && (
                        <span className="flex items-center gap-1 text-gray-500">
                          🎯 {t('reports.bilans_page.labels.objective')}: {camp.objectifParticipations}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="bg-gradient-to-r from-amber-50 to-green-50 rounded-lg p-4 border-l-4 border-amber-500">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2 text-amber-700">
                          ...
                          <span className="font-medium">{t('reports.bilans_page.labels.campaign_success')}</span>
                        </div>
                      ...
                      {camp.objectifParticipations && (
                        <p className="text-sm text-gray-600">
                          {camp.objectifParticipations <= camp.nombreParticipations 
                            ? `✅ ${t('reports.bilans_page.labels.objective')} ${t('reports.bilans_page.labels.reached')} (${camp.nombreParticipations}/${camp.objectifParticipations})`
                            : `📊 ${camp.nombreParticipations}/${camp.objectifParticipations} ${t('reports.bilans_page.labels.participations')}`
                          }
                        </p>
                      )}
