...
const JSONValue = ({ value }: { value: any }) => {
  ...
  if (value === 'N/A') {
    return <span className="text-gray-400 italic text-xs">Non disponible</span>;
  }
      ...
      <div className="space-y-4">
        ...
        {details.timestamp && (
           <div className="flex items-center justify-end gap-2 text-xs text-gray-400 bg-gray-50 dark:bg-gray-900 px-3 py-1 rounded-full w-fit mr-auto">
             <Clock size={12} />
             Horodatage: <span dir="ltr">{new Date(details.timestamp).toLocaleString(locale)}</span>
           </div>
        )}
        ...
        {Object.keys(details).filter(k => !['path', 'reason', 'timestamp', 'success'].includes(k)).length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
             <p className="text-xs font-bold text-gray-500 uppercase mb-2">Autres détails</p>
...
export default function AuditPage() {
  ...
  const getActionLabel = (action: string) => {
    ...
    try {
      ...
      // Prevent crashing by preemptively checking known issues or just rely on try/catch
      // next-intl throws on missing keys in strict mode
      return t(key); 
    } catch (e) {
