...
export async function generateGovernorReport(period: string) {
  try {
    ...
    // 6. News Stats
    const totalActualites = await prisma.actualite.count({
        where: { 
            createdAt: dateFilter,
            isPublie: true 
        }
...
export async function getRecentReportsList(locale: string = 'fr') {
    try {
        ...
        // 1. Fetch real reports from finished activities (Field Reports)
        ...
        fieldActivities.forEach(act => {
            reports.push({
                ...
                subtitle: isAr ? (act.etablissement.nomArabe || act.etablissement.nom) : act.etablissement.nom,
                date: act.dateRapport ? act.dateRapport.toISOString() : act.updatedAt.toISOString(),
                type: 'audit',
                status: isAr ? 'تم التحقق' : 'Vérifié',
                periodValue: act.titre // for identification
            });
        });

        // 2. Add System-Generated Reports (calculated placeholders for monthly reviews)
        ...
        reports.push({
            ...
            title: isAr ? `التقرير الشهRI الإقليمي` : `Rapport Mensuel Provincial`,
