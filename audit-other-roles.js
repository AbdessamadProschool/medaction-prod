...
// Scan roles: autorite, coordinateur, gouverneur
const dirsToScan = [
    'app/[locale]/autorite',
    'app/[locale]/coordinateur',
    'app/[locale]/gouverneur',
    'components/autorite',
    'components/coordinateur',
    'components/gouverneur',
];
...
for (const file of files) {
    ...
    for (const [varName, ns] of Object.entries(varToNs)) {
        // Match t('key') or t.rich('key') or t.raw('key')
        const keyRegex = new RegExp(`${varName}(\\.|(?:\\.rich)|(?:\\.raw))?\\(['"\`]([^'"\`.()\\s]+)['"\`]`, 'g');
        let keyMatch;
        while ((keyMatch = keyRegex.exec(content)) !== null) {
            namespaceMap[ns].add(keyMatch[2]);
        }
    }
}
