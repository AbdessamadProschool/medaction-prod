...
export default function NewsCard({ news, view = 'grid', index = 0 }: NewsCardProps) {
  ...
  let dateText = '';
  if (diffDays === 0) {
    dateText = locale === 'ar' ? 'اليوم' : "Aujourd'hui";
  } else if (diffDays === 1) {
    dateText = locale === 'ar' ? 'أمس' : "Hier";
  } else if (diffDays > 1 && diffDays <= 7) {
    dateText = locale === 'ar' ? `منذ ${diffDays} أيام` : `Il y a ${diffDays} jours`;
  } else {
    dateText = dateObj.toLocaleDateString(locale === 'ar' ? 'ar-MA' : 'fr-FR', {
      day: 'numeric',
      month: 'long',
      year: 'numeric'
    });
