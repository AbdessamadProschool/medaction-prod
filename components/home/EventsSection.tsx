...
export default function EventsSection() {
  ...
  const evenements: Evenement[] = Array.isArray(data?.data?.data)
    ? data.data.data
  ...
  // Countdown text
  const getPreciseStatus = (dateDebutStr: string, dateFinStr?: string): string => {
    const now = new Date();
    const debut = new Date(dateDebutStr);
    // If no end date is provided, let's assume the event lasts 12 hours from the start for "En cours" display
    const fin = dateFinStr ? new Date(dateFinStr) : new Date(debut.getTime() + 12 * 60 * 60 * 1000);

    if (now < debut) {
      const diffTime = debut.getTime() - now.getTime();
      const daysUntil = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
      // if within 24 hours but different day mathematically
      if (daysUntil === 1 && debut.getDate() === now.getDate() + 1) {
        return locale === 'ar' ? 'غداً' : 'Demain';
      }
      if (daysUntil === 1 || daysUntil === 0) {
        return locale === 'ar' ? 'اليوم' : "Aujourd'hui";
      }
      return locale === 'ar' ? `بعد ${daysUntil} أيام` : `Dans ${daysUntil} Jours`;
    } else if (now >= debut && now <= fin) {
      return locale === 'ar' ? 'جارية الآن' : 'En cours';
    } else {
      return locale === 'ar' ? 'منتهية' : 'Terminé';
    }
  };
