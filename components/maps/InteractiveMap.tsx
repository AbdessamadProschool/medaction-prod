...
function MapControls({ 
  ...
  availableAnnexes, selectedAnnexe, onAnnexeChange, // NEW PROPS
  ...
  return (
    <div className="absolute top-4 left-4 z-[1000] flex flex-col gap-3 max-w-[calc(100vw-32px)] md:max-w-[320px]">
         ...
         {!isPublic && (
            ...
            <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                      onCommuneChange(e.target.value ? parseInt(e.target.value) : null);
                      // Annexe change handled by parent effect clearing it, but we start new flow 
                  }}
            ...
            {/* ANNEXE SELECTOR (Conditional) */}
            {selectedCommune && availableAnnexes && availableAnnexes.length > 0 && (
                <div className="relative animate-in fade-in slide-in-from-top-2">
                    <select
                      value={selectedAnnexe || ''}
                      onChange={(e) => onAnnexeChange(e.target.value ? parseInt(e.target.value) : null)}
                      className="w-full appearance-none px-4 py-3 text-sm font-bold bg-slate-50 border-2 border-slate-100 rounded-2xl cursor-pointer text-slate-700 hover:bg-white hover:border-slate-300 transition-all focus:outline-none focus:ring-4 focus:ring-slate-100"
                    >
                      <option value="">{t('filters.all_annexes') || 'Toutes les annexes'}</option>
                      {availableAnnexes.map((a: any) => (
      ...
      {/* Types Scrollable List if needed */}
...
export default function InteractiveMap({ onEtablissementSelect, initialCenter = DEFAULT_CENTER, initialZoom = DEFAULT_ZOOM, height = 'h-full min-h-[70vh]', mode = 'GOVERNOR' }: MapProps) {
  ...
  if (!isMounted || !mapId) return <div className={`relative w-full ${height} bg-slate-100 animate-pulse`} />;
  ...
  return (
    <div className={`relative w-full ${height} overflow-hidden bg-white`}>
      ...
      <MapControls
        ...
        onAnnexeChange={setSelectedAnnexe}
