...
const newGuideDataContent = `export interface StepHighlight {
  top: string;
  left: string;
  width: string;
  height: string;
...
const guideFile = path.join(__dirname, 'app/[locale]/(main)/guide/guideData.ts');
let code = fs.readFileSync(guideFile, 'utf8');

// The tricky part is replacing only the citoyen sections or merging everything correctly.
// Let's use a simpler approach: Extract the exports for 'autorite', 'delegation', 'gouverneur', 'admin'.
// Since evaluating typescript is hard, we can use a regex to replace everything inside `fr: [` and `ar: [`
// However, the best way is to write a script that runs via Node (by compiling it first) to modify the object.
// We installed ts-node, so we can require the TS file, modify the object, and write it back.
// But writing it back as source code is hard.

// Instead, let's just do a string replacement using a custom parser.
// Actually, I can provide the file directly. I'll just write a script that generates the NEW complete guideData.ts
// by keeping the other roles.

// Wait, I will just write a TS file that imports the old one, creates the new object, and writes it using \`JSON.stringify\`.
// But we need it as TypeScript with functions/interfaces... There are no functions, just objects!
// Perfect.

const generatorScript = \`
const fs = require('fs');
const path = require('path');
const { guideData } = require('./app/[locale]/(main)/guide/guideData.backup.js');

const newGuide = \${JSON.stringify(newGuideDataContent)}; // the string above

// We will just read the original file, extract autorite, delegation, gouverneur, admin 
// and append them to our new text? No, it's easier to just use AST or string replace.
\`;

fs.writeFileSync('prepareGuide.js', generatorScript);
